-- ============================================================================
-- 022_product_indexes_and_rls.sql
-- Indexes, Updated At Triggers, and Row Level Security (RLS) for Product Catalog
-- ============================================================================

-- ----------------------------------------------------------------------------
-- 1. INDEXES FOR HIGH-PERFORMANCE CATALOG FILTERING & SEARCH
-- ----------------------------------------------------------------------------
...
-- Public Select Policies (Anyone can browse active catalog)
...
CREATE POLICY "Public read active variants" ON public.product_variants FOR SELECT USING (is_active = TRUE OR public.is_staff_or_admin());
CREATE POLICY "Public read product images" ON public.product_images FOR SELECT USING (TRUE);
CREATE POLICY "Public read product attributes" ON public.product_attributes FOR SELECT USING (TRUE);
...
CREATE POLICY "Staff admin full access images" ON public.product_images FOR ALL USING (public.is_staff_or_admin()) WITH CHECK (public.is_staff_or_admin());
CREATE POLICY "Staff admin full access attributes" ON public.product_attributes FOR ALL USING (public.is_staff_or_admin()) WITH CHECK (public.is_staff_or_admin());
