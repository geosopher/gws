...
-- Fixes syntax error at end of input by ensuring correct termination and clean syntax.
-- ============================================================================
...
-- ============================================================================
-- 6. FUNCTIONS FOR AUTH & AUTHORIZATION
-- ============================================================================
...
CREATE POLICY "Profiles update policy for users"
    ...
    WITH CHECK (
        ...
        AND email_verified = (SELECT p.email_verified FROM public.profiles p WHERE p.id = auth.uid())
        AND deleted_at IS NOT DISTINCT FROM (SELECT p.deleted_at FROM public.profiles p WHERE p.id = auth.uid())
        AND last_login_at IS NOT DISTINCT FROM (SELECT p.last_login_at FROM public.profiles p WHERE p.id = auth.uid())
    );
...
-- ============================================================================
-- END OF ENTERPRISE MIGRATION SCRIPT
-- ============================================================================
